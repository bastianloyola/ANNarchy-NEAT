...
for trial in range(trials):
    ...
    for ep in range(episodes):
        ...
        allretuns[trial-1].append(episode_return)
...
for trial in range(trials):
    ...
    #syn = Projection(pop, pop, target='exc')
    syn = Projection(pop, pop, target='exc', synapse=R_STDP(tau_c=20.0, A_plus=0.01, A_minus=0.01, tau_plus=20.0, tau_minus=20.0))


    syn.connect_from_sparse(matrix)


    compile(directory="ns-cartpole-rstdp")
    j = 0
    returns = []
    actions_done = []
    terminated = False
...
#plot allreturns
numpy_allreturns = np.array(allretuns)
np.save(f"allreturns_{nombre}.npy", numpy_allreturns)

# Graficar los retornos promedio por cada episodio
plt.figure(figsize=(12, 6))
plt.plot(np.mean(numpy_allreturns, axis=0), label='Retorno promedio por episodio')
plt.xlabel('Episodio')
plt.ylabel('Retorno promedio')
plt.title('Retorno promedio por episodio')
plt.legend()
plt.grid()
plt.savefig(f"retornos_promedio_{nombre}.png")
plt.show()
