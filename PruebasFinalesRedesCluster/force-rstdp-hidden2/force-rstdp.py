...
# Índices
total_hidden = n_hidden_layers * nodes_per_hidden
total_nodes = n_inputs + total_hidden + n_outputs
input_index = list(range(n_inputs))
hidden_start = n_inputs
output_index = list(range(hidden_start + total_hidden, total_nodes))

# Inicializar matriz de pesos
matrix = np.zeros((total_nodes, total_nodes))

# Conexiones entrada -> primera capa oculta
for i in input_index:
    for j in range(hidden_start, hidden_start + nodes_per_hidden):
        matrix[i, j] = np.random.uniform(0,110)

# Conexiones entre capas ocultas
for l in range(n_hidden_layers - 1):
    start = hidden_start + l * nodes_per_hidden
    end = start + nodes_per_hidden
    next_start = end
    for i in range(start, end):
        for j in range(next_start, next_start + nodes_per_hidden):
            matrix[i, j] = np.random.uniform(0,110)

# Conexiones última capa oculta -> salida
last_hidden_start = hidden_start + (n_hidden_layers - 1) * nodes_per_hidden
for i in range(last_hidden_start, last_hidden_start + nodes_per_hidden):
...
for trial in range(trials):
    ...
    pop = Population(total_nodes, IZHIKEVICH)
    ...
    compile(directory="ns-forcecartpole-rstdp-33")
    j = 0
    returns = []
    actions_done = []
    terminated = False
