...
largosog = [base_env.LINK_LENGTH_1, base_env.LINK_LENGTH_2]
masasog = [base_env.LINK_MASS_1, base_env.LINK_MASS_2]
moiog = [base_env.LINK_COM_POS_1, base_env.LINK_COM_POS_2]
...
for trial in range(trials):
    ...
    base_env.LINK_LENGTH_1 = largosog[0]
    base_env.LINK_LENGTH_2 = largosog[1]
    base_env.LINK_MASS_1 = masasog[0]
    base_env.LINK_MASS_2 = masasog[1]
    base_env.LINK_COM_POS_1 = moiog[0]
    base_env.LINK_COM_POS_2 = moiog[1]
    ...
    for ep in range(episodes//2):
        ...
        #LINK_LENGTH_1
        if largos != []:
            base_env.LINK_LENGTH_1 = largos[-1][0]
            base_env.LINK_LENGTH_2 = largos[-1][1]
            env.unwrapped.LINK_LENGTH_1 = largos[-1][0]
            env.unwrapped.LINK_LENGTH_2 = largos[-1][1]
        if masas != []:
            base_env.LINK_MASS_1 = masas[-1][0]
            base_env.LINK_MASS_2 = masas[-1][1]
            env.unwrapped.LINK_MASS_1 = masas[-1][0]
            env.unwrapped.LINK_MASS_2 = masas[-1][1]
        if moi != []:
            base_env.LINK_COM_POS_1 = moi[-1][0]
            base_env.LINK_COM_POS_2 = moi[-1][1]
            env.unwrapped.LINK_COM_POS_1 = moi[-1][0]
            env.unwrapped.LINK_COM_POS_2 = moi[-1][1]
        if ep % change_episode == 0:
            change_state["active"] = True
            change_state["remaining"] = 10
        terminated = False
        truncated = False
        episode_return = 0.0
        #print("Comienzo del episodio %d" % (ep + 1))
        ...
        while not terminated and not truncated:
            ...
            pop.reset()
        acciones.append(acciones2)
        largos.append([env.unwrapped.LINK_LENGTH_1, env.unwrapped.LINK_LENGTH_2])
        masas.append([env.unwrapped.LINK_MASS_1, env.unwrapped.LINK_MASS_2])
        moi.append([env.unwrapped.LINK_COM_POS_1, env.unwrapped.LINK_COM_POS_2])
        #print("Episode %d reward: %f" % (ep + 1, episode_return))
        retornos.append(episode_return)
        #print("Recompensas: ", distancias)
        #print("Acciones: ", rs)
...
#plot largo1 y largo2 vs episodio
plt.figure(figsize=(10, 5))
plt.plot([l[0] for l in largos], label='Largo 1', color='blue')
plt.plot([l[1] for l in largos], label='Largo 2', color='orange')
plt.title(f'Largos por episodio - {nombre}')
plt.xlabel('Episodio')
plt.ylabel('Largo')
plt.legend()
plt.tight_layout()
plt.savefig(f'largos_{nombre}.png')
plt.close()
#plot masa1 y masa2 vs episodio
plt.figure(figsize=(10, 5))
plt.plot([m[0] for m in masas], label='Masa 1', color='blue')
plt.plot([m[1] for m in masas], label='Masa 2', color='orange')
plt.title(f'Masas por episodio - {nombre}')
plt.xlabel('Episodio')
plt.ylabel('Masa')
plt.legend()
plt.tight_layout()
plt.savefig(f'masas_{nombre}.png')
plt.close()
#plot moi1 y moi2 vs episodio
plt.figure(figsize=(10, 5))
plt.plot([m[0] for m in moi], label='MOI 1', color='blue')
plt.plot([m[1] for m in moi], label='MOI 2', color='orange')
plt.title(f'MOI por episodio - {nombre}')
plt.xlabel('Episodio')
plt.ylabel('MOI')
plt.legend()
plt.tight_layout()
plt.savefig(f'moi_{nombre}.png')
plt.close()
