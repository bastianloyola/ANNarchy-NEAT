...
update_function = BoundedRandomWalk(scheduler, mu=0, sigma=10, min_val=0.05, max_val=0.5)
#update_function2 = BoundedRandomWalk(scheduler, mu=0, sigma=10, min_val=9.0, max_val=20.0)
#tunable_params = {"length": update_function}
tunable_params = {"masspole": update_function}
...
for trial in range(trials):
    ...
    for ep in range(episodes//2):
        ...
        if largos != []:
            base_env.length = largos[-1]
            env.unwrapped.length = largos[-1]
        if masas != []:
            base_env.masspole = masas[-1]
...
#plot gravedad vs episodio
plt.figure(figsize=(10, 5))
plt.plot(gravedades, label='Gravedad', color='green')
plt.title(f'Gravedad por episodio - {nombre}')
plt.xlabel('Episodio')
plt.ylabel('Gravedad')
plt.legend()
plt.tight_layout()
plt.savefig(f'gravedad_{nombre}.png')
plt.close()

#plot fuerza vs episodio
plt.figure(figsize=(10, 5))
plt.plot(fuerzas, label='Fuerza', color='orange')
plt.title(f'Fuerza por episodio - {nombre}')
plt.xlabel('Episodio')
plt.ylabel('Fuerza')
plt.legend()
plt.tight_layout()
plt.savefig(f'fuerza_{nombre}.png')
plt.close()
